package com.eugenefe.entity;

// Generated 2014. 1. 22 ¿ÀÈÄ 2:10:36 by Hibernate Tools 4.0.0

import javax.persistence.Column;
import javax.persistence.Embeddable;

import com.eugenefe.util.AnnoMethodTree;
import com.eugenefe.util.AnnoNavigationFilter;

/**
 * ProductGreeksId generated by hbm2java
 */
@Embeddable
@AnnoNavigationFilter
public class ProductGreeksId implements java.io.Serializable {

	private String bssd;
	private String pricingObjId;
	private String prodId;
	private String underlyingId;

	public ProductGreeksId() {
	}

	public ProductGreeksId(String bssd, String pricingObjId, String prodId, String underlyingId) {
		this.bssd = bssd;
		this.pricingObjId = pricingObjId;
		this.prodId = prodId;
		this.underlyingId = underlyingId;
	}

	@Column(name = "BSSD", nullable = false, length = 8)
	@AnnoMethodTree(order=10, init=true)
	public String getBssd() {
		return this.bssd;
	}

	public void setBssd(String bssd) {
		this.bssd = bssd;
	}

	@Column(name = "PRICING_OBJ_ID", nullable = false, length = 20)
	@AnnoMethodTree(order=20, init=true)
	public String getPricingObjId() {
		return this.pricingObjId;
	}

	public void setPricingObjId(String pricingObjId) {
		this.pricingObjId = pricingObjId;
	}

	@Column(name = "PROD_ID", nullable = false, length = 20)
	@AnnoMethodTree(order=30, init=true)
	public String getProdId() {
		return this.prodId;
	}

	public void setProdId(String prodId) {
		this.prodId = prodId;
	}

	@Column(name = "UNDERLYING_ID", nullable = false, length = 20)
	public String getUnderlyingId() {
		return this.underlyingId;
	}

	public void setUnderlyingId(String underlyingId) {
		this.underlyingId = underlyingId;
	}

	public boolean equals(Object other) {
		if ((this == other))
			return true;
		if ((other == null))
			return false;
		if (!(other instanceof ProductGreeksId))
			return false;
		ProductGreeksId castOther = (ProductGreeksId) other;

		return ((this.getBssd() == castOther.getBssd()) || (this.getBssd() != null && castOther.getBssd() != null && this
				.getBssd().equals(castOther.getBssd())))
				&& ((this.getPricingObjId() == castOther.getPricingObjId()) || (this.getPricingObjId() != null
						&& castOther.getPricingObjId() != null && this.getPricingObjId().equals(
						castOther.getPricingObjId())))
				&& ((this.getProdId() == castOther.getProdId()) || (this.getProdId() != null
						&& castOther.getProdId() != null && this.getProdId().equals(castOther.getProdId())))
				&& ((this.getUnderlyingId() == castOther.getUnderlyingId()) || (this.getUnderlyingId() != null
						&& castOther.getUnderlyingId() != null && this.getUnderlyingId().equals(
						castOther.getUnderlyingId())));
	}

	public int hashCode() {
		int result = 17;

		result = 37 * result + (getBssd() == null ? 0 : this.getBssd().hashCode());
		result = 37 * result + (getPricingObjId() == null ? 0 : this.getPricingObjId().hashCode());
		result = 37 * result + (getProdId() == null ? 0 : this.getProdId().hashCode());
		result = 37 * result + (getUnderlyingId() == null ? 0 : this.getUnderlyingId().hashCode());
		return result;
	}

}
